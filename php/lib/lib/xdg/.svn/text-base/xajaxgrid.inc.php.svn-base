...
class ScrollTable{
   ...
   var $img_url = 'images';
   /**
    * @var enable_showall - this should be disallowed for tables with massive amounts of records
    */
   var $enable_showall = 0;
   ...
   */
   ...
   function generateHeaderAction($colname){
      // make a default action script for this column
      $formname = $this->formname;
      if ( ($this->use_post) and ($this->formname <> '')) {
         // do a post form
         $thisaction = "onClick='document.forms[\"$formname\"].elements.offset.value=0; document.forms[\"$formname\"].elements.order.value=\"$colname\"; $this->show_funcjs(xajax.getFormValues(\"$formname\"));'";
      } else {
         $thisaction = "onClick='xajax_showGrid(0,$limit,\"$filter\",\"$content\",\"$thiscol\");return false;'";
      }
   ...
   function addRow($table,$arr){
      ...
      if (!$this->read_only) {
         $row .= '<td align="center" width="5%">
                     <a onClick="' . $this->edit_funcjs . '('.$arr[0].',\''.$table.'\');return false;"><img src="'.$this->img_url.'/edit.png" border="0"></a>
                  </td>
                  <td align="center" width="5%">
                     <a href="?" onClick="if (confirm(\'Are you sure?\'))  ' . $this->delete_funcjs . '(\''.$arr[0].'\',\''.$table.'\');return false;"><img src="'.$this->img_url.'/trash.png" border="0"></a>
                  </td>';
         $row .= "</tr>";
      }
      $this->rows .= $row;
   ...
   */
   ...
   function addRowSearch($table,$fieldsFromSearch,$fieldsFromSearchShowAs){
      ...
      $this->search = '
         ...
         <tr>
            <td align="left" width="10%"><button id="submitButton" onClick="' . $this->add_funcjs . '(\''.$table.'\');return false;">Add record</button></td>
      ...
      $this->search .= '
            ...
            &nbsp;&nbsp;<button id="submitButton" onClick="' . $this->scroll_funcjs . '(0,'.$this->numRowsToShow.',document.getElementById(\'searchField\').value,document.getElementById(\'searchContent\').value,document.getElementById(\'searchField\').value);return false;">Continue</button>
            </td>
   ...
   function setPostFooter(){
      ...
      <table class="adminlist">
         <tr>
            <th colspan="'.$this->n_cols.'">
               <span class="pagenav">';
               if($this->start>0){
                  $this->footer .= "<a onClick='document.forms[\"$formname\"].elements.offset.value=0; $this->show_funcjs(xajax.getFormValues(\"$formname\"));'><< First</a>";
               }else{
                  $this->footer .= '<< First';
               }
               $this->footer .= '</span>
               ...
               if($this->start >0){
               $this->footer .= "<a onClick='document.forms[\"$formname\"].elements.offset.value=$previos_rows; $this->show_funcjs(xajax.getFormValues(\"$formname\"));'>< Previous</a>";
               }else{
               ...
               $this->footer .= "<input type='hidden' name='offset' value=$this->start>";
               $this->footer .= "<input type='hidden' name='limit' value=$this->limit>";
               $this->footer .= "<input type='hidden' name='order' value=$this->order>";
               ...
               if($next_rows < $this->numRows){
                  $this->footer .= "<a onClick='document.forms[\"$formname\"].elements.offset.value=$next_rows; $this->show_funcjs(xajax.getFormValues(\"$formname\"));'>Next ></a>";
               }else{
                  $this->footer .= 'Next >';
               }
               ...
               if($next_rows < $this->numRows){
                  $lastpage = $this->numRows - $this->limit;
                  
                  $this->footer .= "<a onClick='document.forms[\"$formname\"].elements.offset.value=$lastpage; $this->show_funcjs(xajax.getFormValues(\"$formname\"));'>Last >></a>";
               }else{
      ...
      $this->search = "
               ...
               <tr>
                  ...
                  <td>Show <input type=text name=\"limit1\" value=$this->limit size=8> Records per Page 
                  <input type=button id=\"refreshButton1\" onClick='document.forms[\"$formname\"].elements.limit.value=document.forms[\"$formname\"].elements.limit1.value; document.forms[\"$formname\"].elements.limit2.value=document.forms[\"$formname\"].elements.limit1.value; " . $this->show_funcjs . "(xajax.getFormValues(\"$formname\"));' value='Show'>";
                  
      if ($this->enable_showall) {
         $this->search .= " | 
         <input type=button id=\"submitButton\" onClick='$this->show_funcjs(xajax.getFormValues(\"$formname\"));' value='Show All'>";
      }
      $this->search .= "
                  </td>
               </tr>
      ...
      $this->search2 = "
               ...
               <tr>
                  <td>XajaxGrid V 1.0</td>
                  <td>Show <input type=text name=\"limit2\" value=$this->limit size=8> Records per Page 
                  <input type=button id=\"refreshButton2\" onClick='document.forms[\"$formname\"].elements.limit.value=document.forms[\"$formname\"].elements.limit2.value; document.forms[\"$formname\"].elements.limit1.value=document.forms[\"$formname\"].elements.limit2.value; " . $this->show_funcjs . "(xajax.getFormValues(\"$formname\"));' value='Show'>";
      ...
      if ( ($this->use_post) and ($this->formname <> '')) {
         ...
         <table class="adminlist">
            <tr>
               <th colspan="'.$this->n_cols.'">
                  <span class="pagenav">';
                  if($this->start>0){
                     $this->footer .= '<a href="?" onClick=\'' . $this->show_funcjs . '(0,'.$this->limit.',"'.$this->filter.'","'.$this->content.'","'.$this->order.'");return false;\'><< First</a>';
                  }else{
                     $this->footer .= '<< First';
                  }
                  $this->footer .= '</span>
                  ...
                  if($this->start >0){
                  $this->footer .= '
                     <a href="?" onClick=\'' . $this->show_funcjs . '('.$previos_rows.','.$this->limit.',"'.$this->filter.'","'.$this->content.'","'.$this->order.'");return false;\'>< Previous</a>';
                  }else{
                     $this->footer .= '< Previous';
                  }
                  $this->footer .= '
                  ...
                  if($next_rows < $this->numRows){
                     $this->footer .= '<a href="?" onClick=\'' . $this->show_funcjs . '('.$next_rows.','.$this->limit.',"'.$this->filter.'","'.$this->content.'","'.$this->order.'");return false;\'>Next ></a>';
                  }else{
                     $this->footer .= 'Next >';
                  }
                  ...
                  if($next_rows < $this->numRows){
                  $this->footer .= '<a href="?" onClick=\'' . $this->show_funcjs . '('.($this->numRows - $this->limit).','.$this->limit.',"'.$this->filter.'","'.$this->content.'","'.$this->order.'");return false;\'>Last >></a>';
                  }else{
                  $this->footer .= 'Last >></span>';
                  }
               $this->footer .= '
         ...
         $this->footer .= $this->search = '
            ...
            <tr>
               <td>XajaxGrid V 1.0</td>
               <td align="right" >
               <button id="submitButton" onClick="' . $this->show_funcjs . '(0,'.MAXROWSXPAGE.');return false;">Show All</button>
               </td>
            </tr>
         </table>';
      }
