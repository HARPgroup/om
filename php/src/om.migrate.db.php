...
if (count($args) >= 2) {
  ...
  error_log("If query_type = check and hydrocode is integer, will assume a pid for the model element that is the parent of the model element has been submitted, if hydrocode is not an integer, it will query to find the element with a matching om_element_connection");
  die;
}
...
foreach ($data as $element) {
  ...
  if (is_object($om_object)) {
    $dh_model = om_load_dh_model($query_type, $om_fid, $om_object->name, $model_varkey, $om_object->object_class, $model_scenario);
