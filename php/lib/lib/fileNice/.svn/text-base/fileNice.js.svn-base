...
// modified by RWB to allow files other than index.php to be used
if(!fnscript) var fnscript = 'index.php';
...
com.filenice.actions = function(){
	...
	this.getFolder = function(){
		...
		if(element.className == "contents") {
			...
			var url= fnscript + "?action=getFolderContents&dir="+dir+"&containerid"+containerid;
            // END modification by RWB
...
function getNextImage(currentPic){
	...
	var url= fnscript + "?action=nextImage&pic="+currentPic;
	// END modification by RWB
	if (window.XMLHttpRequest){ // moz
		request = new XMLHttpRequest();
	}else if (window.ActiveXObject){ // IE
...
// add in javascript to handle requests, so that multiple instances can be defined
// added by RWB 12.19.2007
function setFNFunctions () {
   var o = new com.filenice.actions;
   //alert('Entering setFNFunctions');
   var allDivs = document.getElementsByTagName("div");
   var len = allDivs.length;
   //alert('Found ' + len + ' divs');
   for(var x=0;x<len;x++){
      var singleDiv = allDivs[x];
      var divid = singleDiv.id;
      //alert(singleDiv.id + ' ' + divid.substring(0,10));
      if ( divid.substring(0,11) == 'fnContainer') {
		  //alert('Executing setFunctions');
		  o.setFunctions(document.getElementById(singleDiv.id));
	  }
   }
