   ...
   if (is_object($thisobject)) {
      ...
      if (property_exists($thisobject, 'processors')) {
        ...
        foreach ($thisobject->processors as $comp_name => $thisproc) {
          ...
          error_log("Saving $comp_name operator on $elid as ID $operatorid with class = $oc");
...
function extract_xml_sertag($thisobject) {
  ...
  foreach ($props as $propname) {
    #error_log($thisprop);
    if (property_exists($propname, $thisobject)) {
...
function unSerializeSingleModelObject($elementid, $input_props = array(), $debug = 0, $runtime_db = FALSE, $cached = FALSE, $cache_runid = -2 ) {
  ...
  foreach ($linkrecs as $thisrec) {
    ...
    if ($debug) {
      $returnArray['debug'] .= " Searching for $src_id in " . print_r(array_keys($unserobjects),1) . '<br>';
    }
    if ($src_id <> -1) {
