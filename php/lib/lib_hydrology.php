...
class modelObject {
  ...
  function setPropJSON2d($propname, $propvalue, $view = '') {
    ...
    foreach ($json_props as $pname => $pvalue) {
      error_log("setPropJSON2d($this->name): this->setPropJSON2d($pname)");
...
class broadCastObject extends modelSubObject {
  ...
  function getPublicProps() {
    ...
    error_log("Calling getPublicProps() on $this->name");
    error_log("- broacast_mode = $this->broadcast_mode");
    error_log("- local var names = " . print_r($this->local_varname,1));
...
class dataMatrix extends modelSubObject {
   ...
   var $valuetype = 1; // 0 - returns entire array (normal), 1 - single column lookup (col), 2 - 2 column lookup (col & row)
  ...
  function init() {
    ...
    $this->formatMatrix();
    $this->recordInvars();
    // valuetype 0, aka "normal" is crazy.  need to assume it is returning an array which needs to be handled
...
class hydroImpoundment extends hydroObject {
   ...
   function wake() {
      ...
      $this->prop_desc['precip_mgd'] = 'Calculated rate of precipitation on the lakes surface (MGD).';
